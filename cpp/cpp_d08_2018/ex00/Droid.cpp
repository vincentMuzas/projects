...
Droid::Droid(const Droid &old) : Id(old.getId()),Energy(50),Attack(25), Toughness(15)
{
	this->setStatus(new std::string("Standing by"));
	std::cout << "Droid '"<< Id <<"' Activated, Memory Dumped" << std::endl;
}

bool Droid::operator==(const Droid &rop) const
{
	if (Id == rop.Id && Energy == rop.Energy &&
	Attack == rop.Attack && Toughness == rop.Toughness
	&& Status->compare(*rop.Status)) {
		return (true);
	}
	return (false);
}
